...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		newBatchPoster, err := prompt.EnterAddress("enter address for revoke authorization batch poster: ")
		if err != nil {
			log.Fatal(err)
		}

		response, err := UpgradeExecutor.SetIsBatchPoster(auto, common.HexToAddress(*newBatchPoster), false) // revoke
		if err != nil {
			log.Fatal(err)
		}
